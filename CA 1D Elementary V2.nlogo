...
to go
  ...
  let prev-n-off n-switched-off
  set n-switched-off 0
  ...
  if ticks > 1 [
    set rel-change 0
    if prev-n-off != 0 [
      set rel-change (abs (n-switched-off - prev-n-off) / prev-n-off)
    ]
  ]
...
to do-rule  ;; patch procedure
  ...
  if on? and not new-value [
    set n-switched-off n-switched-off + 1
  ]
...
to-report patch-string
  let state-list map [ x ->
    [on?] of patch x min-pycor
  ] n-values world-width [ x -> min-pxcor + x ]

  report reduce [[ so-far next? ] ->
    (word so-far (ifelse-value next? ["1"] ["0"]))
  ] fput "" state-list
end
...
to-report patch-substring-counts [ N ]
  ...
  while [ length remaining >= N ] [
    let str substring remaining 0 N

    ifelse table:has-key? t str [
      table:put t str (1 + table:get t str)
    ] [
      table:put t str 1
    ]

    set remaining but-first remaining
  ]
...
to-report entropy [ N ]
  let t patch-substring-counts N

  let H 0
  foreach table:keys t [ s ->
    let p (table:get t s) / world-width
    set H H - (p * log p 2)
  ]
...
to-report effective-measure-complexity [ max-N ]

  let N 2
  let h-list []
  let last-entropy entropy 1

  while [ N <= max-N ] [
    let this-entropy entropy N
    set h-list lput (this-entropy - last-entropy) h-list
    set N N + 1
    set last-entropy this-entropy
  ]

  let h last h-list

  report reduce [[ so-far next ] -> so-far + next - h] h-list
end
@#$#@#$#@
GRAPHICS-WINDOW
229
...
true
false
"clear-plot" "if all? (patches with [pycor = min-pycor]) [is-boolean? on?] [\n  set-current-plot-pen \"default\"\n  foreach n-values 100 [ i -> i + 1 ] [ N ->\n    plot entropy N\n  ]\n]"
PENS
...
387
EMC
ifelse-value ticks >= 128 [\n  effective-measure-complexity 100\n] [\n  \"N/A\"\n]
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="EMC-experiment" repetitions="10" runMetricsEveryStep="false">
    <setup>setup-random</setup>
    <go>go</go>
    <timeLimit steps="128"/>
    <metric>effective-measure-complexity 100</metric>
  ...
  <experiment name="relative-change" repetitions="10" runMetricsEveryStep="false">
    <setup>setup-random</setup>
    <go>go</go>
    <timeLimit steps="1024"/>
    <metric>rel-change</metric>
    <runMetricsCondition>ticks &gt;= 2</runMetricsCondition>
    <enumeratedValueSet variable="density">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rule">
